...
import { DEFAULT_STEPPER_STATE } from "./_data/StepperPresets";
...
import { SECTIONS, type SectionId, type StepperState, type StudioPreset } from "./types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<StepperState>(DEFAULT_STEPPER_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const [activePresetId, setActivePresetId] = useState<string | null>(null);
  const [downloadName] = useState("stepper-component");
  ...
  const update = <K extends keyof StepperState>(key: K, value: StepperState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_STEPPER_STATE, ...(preset.state as Partial<StepperState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Stepper Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
